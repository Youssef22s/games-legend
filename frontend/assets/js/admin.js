...
document.getElementById('addVendorForm').addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            showAlert(result.error || 'Failed to add vendor', 'danger');
        }
    } catch (error) {
...
async function deleteVendor(id) {
    if (confirm('Are you sure you want to remove this vendor from the platform?')) {
        try {
            ...
            showAlert('Error while deleting', 'danger');
        }
    }
}
